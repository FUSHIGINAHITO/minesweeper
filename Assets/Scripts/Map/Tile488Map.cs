...
public class Tile488Map : PeriodicMotifMap
{
    ...
    protected override void BuildPattern(float s, out Vector2 b1, out Vector2 b2, out MotifCell[] motif)
    {
        ...
        motif = new[]
        {
            new MotifCell(CellShapeType.Octa, Vector2.zero, 0f),
            new MotifCell(CellShapeType.Octa, new Vector2(0.5f * L, 0.5f * L), 0f),
            new MotifCell(CellShapeType.Square, new Vector2(0.5f * L, 0f), 0f),
            new MotifCell(CellShapeType.Square, new Vector2(0f, 0.5f * L), 0f)
        };
    }
}