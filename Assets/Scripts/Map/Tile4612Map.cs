...
public class Tile4612Map : PeriodicMotifMap
{
    ...
    protected override void BuildPattern(float s, out Vector2 b1, out Vector2 b2, out MotifCell[] motif)
    {
        ...
        {
            // 1 × dodec
            new MotifCell(CellShapeType.Dodeca, Vector2.zero, 0f),

            // 2 × hex（分数坐标：1/3,1/3 与 2/3,2/3）
            new MotifCell(CellShapeType.Hex, b1b2 / 3f, 0f),
            new MotifCell(CellShapeType.Hex, b1b2 * (2f / 3f), 0f),

            // 3 × square（分数坐标：1/2,0 ; 0,1/2 ; 1/2,1/2）
            new MotifCell(CellShapeType.Square, b1 * 0.5f, 0f),
            new MotifCell(CellShapeType.Square, b2 * 0.5f, 60f),
            new MotifCell(CellShapeType.Square, b1b2 * 0.5f, 120f)
        };
    }
}